...
class PrecomputedTextEmbeddingDataset(Dataset):
    """Read variable-length BF16 prompt embeddings from safetensors shards."""

    def __init__(self, data_path: str):
        self.root = Path(data_path)
        manifest_path = self.root / "manifest.json"
        index_path = self.root / "index.jsonl"
        prompts_path = self.root / "selected_prompts.txt"

        for path in (manifest_path, index_path, prompts_path):
            if not path.is_file():
                raise FileNotFoundError(f"Missing precomputed text artifact: {path}")

        with manifest_path.open("r", encoding="utf-8") as f:
            self.manifest = json.load(f)
        if self.manifest.get("dtype") != "bfloat16":
            raise ValueError(
                f"Expected bfloat16 embeddings, got {self.manifest.get('dtype')}"
            )

        with prompts_path.open("r", encoding="utf-8") as f:
            self.prompt_list = [line.rstrip("\n") for line in f]
        with index_path.open("r", encoding="utf-8") as f:
            self.index = [json.loads(line) for line in f if line.strip()]

        expected_count = int(self.manifest["selected_prompt_count"])
        if len(self.prompt_list) != expected_count or len(self.index) != expected_count:
            raise ValueError(
                "Prompt/index count mismatch: "
                f"prompts={len(self.prompt_list)}, index={len(self.index)}, "
                f"manifest={expected_count}"
            )

        for position, record in enumerate(self.index):
            if int(record["index"]) != position:
                raise ValueError(
                    f"Embedding index is not dense at position {position}: {record}"
                )
    ...
    def __getitem__(self, idx):
        ...
        with safe_open(str(shard_path), framework="pt", device="cpu") as shard:
            prompt_embeds = shard.get_tensor(record["key"]).clone()

        expected_length = int(record["length"])
        if prompt_embeds.dtype != torch.bfloat16:
            raise TypeError(
                f"Embedding {idx} has dtype {prompt_embeds.dtype}, expected bfloat16"
            )
        if prompt_embeds.ndim != 2 or prompt_embeds.shape[0] != expected_length:
            raise ValueError(
                f"Embedding {idx} has shape {tuple(prompt_embeds.shape)}, "
                f"expected [{expected_length}, hidden_dim]"
            )
